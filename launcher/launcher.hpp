...
private:
  ...
  void **_ptr;
public:
  OclArg(size_t size, ArgType argType, cl_mem_flags clMemFlags, void **ptr =
      NULL) :
...
public:
  inline Launcher(unsigned int min, unsigned int max, unsigned int dim,
      unsigned int step) :
      _min(min), _max(max), _dim(dim), _step(step), _best(NULL), _isInit(false), _numArgs(
          0) {
    init();
  ...
  long long unsigned int getNumberOfExecutions() {
