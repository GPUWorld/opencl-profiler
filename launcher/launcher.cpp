...
void Launcher::initKernel(const char *filename, const char *kernelName) {
  ...
  err = clBuildProgram(_program, 0, NULL,
      "", NULL, NULL);
...
Execution& Launcher::runKernel(size_t *localWorkSize) {
  ...
  clCheckError(
      clEnqueueNDRangeKernel(_commandQueue, _kernel, _dim, 0, _global_work,
          localWorkSize, 0, NULL, &ev), "Error on kernel launch");

  clWaitForEvents(1, &ev);

  clCheckError(
      clGetEventProfilingInfo(ev, CL_PROFILING_COMMAND_START, sizeof(cl_ulong),
          &startTime, NULL), "Error reading start execution");
  clCheckError(
      clGetEventProfilingInfo(ev, CL_PROFILING_COMMAND_END, sizeof(cl_ulong),
          &endTime, NULL), "Error reading end execution");
...
void Launcher::initDevices() {
  if (_platforms.size() != 0) {
    ...
    for (unsigned int j = 0; j < _platforms.size(); j++) {
      clCheckError(
          clGetDeviceIDs(_platforms[j], CL_DEVICE_TYPE_ALL, 0, NULL,
              &numDevices), "DEVICE CHECK 1 ERROR\n");

      cl_device_id *clDevices = new cl_device_id[numDevices];

      clCheckError(
          clGetDeviceIDs(_platforms[j], CL_DEVICE_TYPE_ALL, numDevices,
              clDevices, NULL), "DEVICE CHECK 2 ERROR\n");
...
void Launcher::initPlatforms() {
  ...
  clCheckError(clGetPlatformIDs(NULL, NULL, &numPlatforms),
      "PLATFORM CHECK 1 ERROR\n");

  cl_platform_id *clPlatforms = new cl_platform_id[numPlatforms];

  clCheckError(clGetPlatformIDs(numPlatforms, clPlatforms, NULL),
      "PLATFORM CHECK 2 ERROR\n");
...
void Launcher::initCommandQueue() {
  ...
  if (_context != NULL && &_currentDevice != NULL) {
    _commandQueue = clCreateCommandQueue(_context, _currentDevice,
...
void Launcher::allocAndSet(OclArg arg) {
  cl_mem buffer;
  int err;
  buffer = clCreateBuffer(getContext(), arg.getClMemFlag(), arg.getSize(), NULL,
      &err);
  clCheckError(err, "Error creating buffer");
  clCheckError(clSetKernelArg(getKernel(), _numArgs++, sizeof(cl_mem), &buffer),
...
void Launcher::set(OclArg arg) {
  clCheckError(
      clSetKernelArg(getKernel(), _numArgs++, arg.getSize(), arg.getPtr()),
      "Error setting argument");
}
...
void Launcher::printStats() {
  ...
  case 2:
    ...
    break;
  case 3:
    cout << "x=" << getBest()->getX() << ", y=" << getBest()->getY() << ", z="
        << getBest()->getZ() << endl;
    break;
  default:
    throw;
